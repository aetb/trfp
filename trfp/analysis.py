...
def blind_moments(moment_dfs, keys, blinds):
    ...
    for key in keys:
        ...
        elif key[:4] == 'book': blinded_moment_dfs[key] = _apply_blinds_tr(moment_dfs[key], blinds)
...
def moment_to_corrected(moment_dfs, keys):
    corrected_dfs = {}
    for key in keys:
        if (key[:2] == 'tr') or (key[:4] == 'book'): 
            print "\nRemoving trolley footprints for " + key
            try:
                corrected_dfs[key] = trolley_footprint_replacement(moment_dfs[key])
            except:
                corrected_dfs[key] = moment_dfs[key].copy()
                print '\nCould not remove footprint for key: ' + key
