...
def calc_moment_df(interp_df, trolleyProbes = np.zeros(17)+1,\
    ...
    FP6Theta = ProbeDrops(FP6Geometry, 14, 5) #14 trm and m5 for fp
        
    FP4Geometry = Geometry(trfp.FP4_X, trfp.FP4_Y, np.zeros(4)+1)
    FP4Theta = ProbeDrops(FP4Geometry, 14, 4) #same as FP6Theta
    ...
    if tr_run:
        ...
        TrolleyTheta = ProbeDrops(TrolleyGeometry, tpMomentCap=tpMomentCap)
    ...
    for station in np.arange(72):
        ...
        if (len(trfp.STATION_PROBE_ID[station]) == 6):
            if station <=5:
                StGeom = Geometry(trfp.FP6_X+1, trfp.FP6_Y, fixedProbes[station])
                StTheta = ProbeDrops(StGeom, fpMomentCap=fpMomentCaps[station])
                theta_fp = StTheta
                
            elif not(0 in fixedProbes[station]) and (fpMomentCaps[station] == 5):
                theta_fp = FP6Theta
                #theta_fp = trfp.THETA_FP_6
            else: 
            ...
            St41Theta = ProbeDrops(St41Geom, fpMomentCap=fpMomentCaps[station])
            theta_fp = St41Theta
        
        elif (station == 37) or (station == 39):
            St37_39Geom = Geometry(trfp.FP4_X_ST37_ST39, trfp.FP4_Y,\
                                   fixedProbes[station][:4])
            St37_39Theta = ProbeDrops(St37_39Geom, fpMomentCap=fpMomentCaps[station])
            theta_fp = St37_39Theta
            
        else:
            if station <= 5:
                StGeom = Geometry(trfp.FP4_X+1, trfp.FP4_Y, fixedProbes[station])
                StTheta = ProbeDrops(StGeom, fpMomentCap=fpMomentCaps[station])
                theta_fp = StTheta
                
            elif (not (0 in fixedProbes[station][:4])) and (fpMomentCaps[station] == 5):
                theta_fp = FP4Theta
            else:
